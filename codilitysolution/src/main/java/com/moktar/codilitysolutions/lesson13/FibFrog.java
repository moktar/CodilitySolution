...
public class FibFrog {
    ...
    public int solution(int[] A) {
        ...
        int[] points = new int[A.length + 1];
        points[points.length - 1] = -1;
        for (int i = 0; i < points.length; i++) {
            if (fibonacciCounters[i + 1] && A[i] == 1) {
                points[i] = 1;
            }
        }
        ...
        for (int i = 0; i < points.length - 1; i++) {
            ...
            for (int f : fibonacci) {
                int idx = i + f;
                int jump = p + 1;
                if (0 < idx && idx < points.length
                        && (idx == A.length || A[idx] == 1)
                        && (points[idx] < 1 || jump < points[idx])) {
                    points[idx] = jump;
                }
    ...
    private int[] prepareFibonacciNumbers() {
        int max = 26;
